...
public class ChangeToFolderInShellAction implements IObjectActionDelegate {
	...
	private static final ShellLogger shellLogger = new ShellLogger(
			ChangeToFolderInShellAction.class);
	...
	public void run(IAction action) {
		...
		if(!ShellViewUtil.isTargetAvailable()) {
			...
			if(MessageDialogHandler.openConfirmationMessage(targetPart.getSite().getShell(),
					"There is no Shell available, do you want to open a new one?")) {
				try {
					shellView = (ShellView) targetPart.getSite().getPage().showView(
							ShellID.SHELL_VIEW_ID);
				}
		...
		if(shellView != null) {
			IShellDescriptor shellDescriptor = shellView.getShellViewer().getShellFacade()
					.getShellDescriptor();
			...
			String changeDirectoryCommand = commandProvider.getChangeDirectoryCommand(
	...
	public void selectionChanged(IAction action, ISelection selection) {
		...
		if(!structuredSelection.isEmpty()) {
			...
			if(selectedElement instanceof IResource) {
				currentResource = (IResource) selectedElement;
				action.setEnabled(currentResource.exists());
			}
			else {
