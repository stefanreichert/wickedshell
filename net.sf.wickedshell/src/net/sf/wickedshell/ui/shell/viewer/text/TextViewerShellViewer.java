 ...
 * @since 18.10.2006
 */
public class TextViewerShellViewer implements IShellViewer, IShellListener {

	/** The <code>Reader</code> of the shell. */
	private final ShellLogger shellLogger = new ShellLogger(TextViewerShellViewer.class);

	/** The <code>Text</code> functioning as display. */
	private TextViewer shellDisplayTextViewer;

	private StyledText shellDisplayControl;
	 ...
	 *            The <code>Text</code> functioning as display
	 */
	public TextViewerShellViewer(Composite parent, int style) {
		this(new TextViewer(parent, style));
	}
	 ...
	 *            The <code>Text</code> functioning as display
	 */
	public TextViewerShellViewer(final TextViewer shellDisplayTextViewer) {
		super();
		this.shellDisplayTextViewer = shellDisplayTextViewer;
		this.shellDisplayControl = shellDisplayTextViewer.getTextWidget();
		refreshFont();
		refreshForeground();
		refreshBackground();
		initFieldAssist();
		this.lastLine = new String();
		this.shellDisplayTextViewer.setDocument(new Document());
		this.shellDisplayControl.addModifyListener(new ModifyListener() {
			public void modifyText(ModifyEvent event) {
				int lastLineStart = shellDisplayTextViewer.getDocument().get().lastIndexOf(shellDisplayTextViewer.getTextWidget().getLineDelimiter());
				if(lastLineStart == -1) {
					lastLine = shellDisplayTextViewer.getDocument().get();
				}
				else {
					lastLine = shellDisplayTextViewer.getDocument().get().substring(lastLineStart + shellDisplayControl.getLineDelimiter().length());
				}
			}
		});
		ShellPlugin.getDefault().getPreferenceStore().addPropertyChangeListener(new org.eclipse.jface.util.IPropertyChangeListener() {
			...
			public void propertyChange(PropertyChangeEvent event) {
				if(!TextViewerShellViewer.this.shellDisplayControl.isDisposed()) {
	...
	public int getCurrentCaretPosition() {
		return shellDisplayControl.getCaretOffset();
	}
	...
	public String getCurrentCommand() {
		return shellDisplayTextViewer.getDocument().get().substring(getCurrentCommandOffset());
	}
	...
	public int getCurrentCommandOffset() {
		if(lastLine.indexOf(shellFacade.getShellDescriptor().getPathDelimiter()) != -1) {
			...
			return shellDisplayTextViewer.getDocument().get().length() - (lastLine.length() - getCurrentPath().length() - delimiterLength);
		}
		return shellDisplayTextViewer.getDocument().get().length() + 1;
	}
	...
	public boolean isCurrentCaretPositionValid() {
		return shellDisplayTextViewer.getSelectedRange().x >= getCurrentCommandOffset();
	}
	...
	public void setCurrentCommand(String command) {
		try {
			shellDisplayTextViewer.getDocument().replace(getCurrentCommandOffset(), getCurrentCommand().length(), command);
			shellDisplayTextViewer.invalidateTextPresentation();
		}
	...
	public void clear() {
		...
		shellDisplayTextViewer.getDocument().set("");
	...
	public int getContentLength() {
		return shellDisplayTextViewer.getDocument().getLength();
	}
	...
	public String getViewerContent() {
		return shellDisplayTextViewer.getDocument().get();
	}
	...
	private class CommandHistoryDialog extends PopupDialog {
		...
		protected void adjustBounds() {
			...
			Point location = shellDisplayControl.getDisplay().map(shellDisplayControl, null, shellDisplayControl.getCaret().getLocation());
			int commandLength = 0;
