 ...
 */
public class CmdExternalShellInvoker implements IExternalShellInvoker {

	/** The <code>Reader</code> of the shell. */
	private final ShellLogger shellLogger = new ShellLogger(
			CmdExternalShellInvoker.class);
	...
	public void execute(IShellDescriptor descriptor, String command, String path) {
		try {
			Runtime.getRuntime().exec("cmd.exe /C start cmd.exe /K " + command,
					null, new File(path));
		}
